...
function getScore() {
	...
	if (httpObject != null) {
		var url = "../scorebord/scoreophalen.php?&pid="+Math.random();
...
function setScorebord() {
	if(httpObject.readyState == 4){
		...
		//document.getElementById("laatsterefresh").innerHTML = "Laatste refresh: "+hours+":"+minutes+":"+seconds;
	}
}
...
function deleteSessie() {
	httpObject = getHTTPObject();
	if (httpObject != null) {
		var url = "../scorebord/deletesessie.php?&pid="+Math.random();
		httpObject.open("GET", url, true);
		httpObject.send(null); 
	}
}	
